...
This guide covers the complete recording workflow—from starting the recording process to using advanced automation tools and assertions.
...
Before the recording starts, configure the environment in which the test will be executed.
...
### Target URL
Enter the URL of the application to be tested. The browser will open this URL when the recording starts.

### Device
Select a predefined device preset or add a custom device configuration. This defines the screen resolution and viewport size.
...
- Used to easily bypass login steps.
...
- The browser is controlled entirely by Robonito and dedicated to the active recording session.
...
Each interaction is automatically converted into a test step in your test case.
...
### Page Assertion
Page Assertions are used to validate page-level properties during test execution. They ensure that the application navigates to the correct page and that the page loads exactly as expected.  
[Learn Page Assertion](/docs/assertions/page-assertion)

### Visual Assert
Validates the visual appearance of UI elements or entire pages to detect unexpected layout or UI changes.

### Hover
Records mouse hover interactions for menus, tooltips, and dynamic UI elements.

### Record Local Variable
Captures values directly from the UI and stores them as variables for later use in the test.
...
### Optional Step
Marks a step as optional, allowing the test to continue execution even if that specific step fails.
...
3. The test case is now ready for review, editing, and execution.

## Benefits of Recording-Based Testing
...
- Reduced test maintenance through smart assertions and variables.
...
After stopping the recording or completing an execution flow, Robonito converts all recorded user interactions into structured, readable test steps.
...
- **Action Type Tag:** Identifies the type of action (e.g., Browser, Navigate, Click).

### Example Recorded Steps

- **Open Browser:** Initializes a new browser session for the test case.
- **Navigate to URL:** Opens the specified target URL in the active browser.
- **Click Action:** Captures a user click on a specific UI element.
- **Additional Actions:** Records subsequent user interactions in logical sequence.

## Step Actions and Capabilities

Recorded steps can be:
- Reordered using simple drag-and-drop.
- Searched using the step search bar.
- Edited to refine selectors, inputs, or values.
- Enhanced with assertions, variables, or optional behavior.

Each step is modular and reusable, giving you precise control over test execution.

## Why Recorded Steps Matter

Structured recorded steps provide:
- Full visibility into the test execution flow.
- Easier debugging and test maintenance.
- Complete flexibility to enhance tests after recording.
