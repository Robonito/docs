...
4. The **Edit Event Node** panel will open.
...
This field indicates the category of action being performed. It helps identify whether the step is a click, input, navigation, or assertion.
...
- Increase the timeout for slow-loading elements.
- Decrease the timeout for faster failure detection.
...
When enabled:
- The step will not fail the testcase if it fails.
...
> **Note:** Avoid overusing this option, as it can hide real issues.
...
Adding meaningful descriptions improves the readability and maintainability of test cases.
...
Robonito compares the current UI snapshot with the stored baseline image. If differences exceed the allowed tolerance, the step fails.
...
Displays the stored baseline image reference used for visual comparison. This reference image is used to validate UI consistency across executions.
...
The **Advanced Settings** section allows further customization of element behavior and matching logic (if applicable). These settings help fine-tune how Robonito interacts with or validates an element.
